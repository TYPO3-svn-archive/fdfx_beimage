...
/*		var actionRotateLeft = $('#actionRotateLeft');
		var actionRotateRight = $('#actionRotateRight');	*/
/*		$(actionRotateLeft).removeAttr('disabled');
		$(actionRotateRight).removeAttr('disabled');
		$(actionRotateLeft).removeClass('button disabledButton');
		$(actionRotateRight).removeClass('button disabledButton');
		$(actionRotateLeft).addClass('button');
		$(actionRotateRight).addClass('button');	*/
...
/*		var actionRotateLeft = $('#actionRotateLeft');
		var actionRotateRight = $('#actionRotateRight');
		$(actionRotateLeft).attr('disabled', 'disabled');
		$(actionRotateRight).attr('disabled', 'disabled');
		$(actionRotateLeft).removeClass('button disabledButton');
		$(actionRotateRight).removeClass('button disabledButton');
		$(actionRotateLeft).addClass('disabledButton');
		$(actionRotateRight).addClass('disabledButton');	*/
	}
				...
				success: function(data)
				{
										...
										}else
										{
											$("#loading").show();
												...
												/*PRS+ 2007 12 11 */
												if (numSessionHistory==0)
												{
													clearImageHistory();
												}
												/*PRS- 2007 12 11 */
									        var preImage = new Image();
													preImage.width = parseInt(data.width);
													preImage.height = parseInt(data.height);
									        ...
									        update(data.image_ref,data.chash)
										}
				}
			};
...
function update(image,chash)
{
	document.getElementById('path').value=image;
	setCHash(chash);
	alert(imagesaved+image);
}
function saveImage()
{
	if (!isImageHistoryExist())
